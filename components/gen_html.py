...
# In the consolidated repo, this script lives in components/ and writes its
# output there directly (as full-list.html) rather than into a docs/
# subfolder — components/ IS the served directory for this page.
DOCS_DIR = ROOT
HTML_PATH = DOCS_DIR / "full-list.html"
...
def sync_workbook() -> None:
    """Copy the source .xlsx from the repo's top-level docs/ folder into
    components/ so the page's client-side fetch('RDK-B_Component_List_2026.xlsx')
    — a same-directory relative path — resolves once this is served by
    GitHub Pages. Always re-copies (not just "if missing") so a newer xlsx
    in docs/ is picked up on every build."""
    dest = DOCS_DIR / WORKBOOK_NAME
    source = ROOT.parent / "docs" / WORKBOOK_NAME
    if not source.exists():
        # fall back to the old same-folder convention, in case this script
        # is ever run standalone outside the consolidated repo layout
        source = ROOT / WORKBOOK_NAME
    if not source.exists():
        print(f"WARNING: no {WORKBOOK_NAME} found in ../docs/ or {ROOT}. "
